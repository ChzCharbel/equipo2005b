...
module.exports = class Usuario{
    ...
    static isUser(idIVD) {
        getUserById(idIVD)
        .then((usuarioIVD) => {
            if (usuarioIVD.status != 'success' || usuarioIVD.data.status != 'active') {
                return false;
            }
            else{
                return true;
    ...
    static getUsername(idIVD) {
        ...
        .then((usuarioIVD) => {
            if (this.isUser() == true) {
                console.log(usuarioIVD.data.name + ' ' + usuarioIVD.data.first_surname + ' ' + usuarioIVD.data.second_surname); 
                const nombre = usuarioIVD.data.name + ' ' + usuarioIVD.data.first_surname + ' ' + usuarioIVD.data.second_surname;
                return nombre;
            }
        })
        .catch((error) => {
    ...
    static fetchOne(matricula) {
        return db.query(`Select * from "Usuario" Where "idIVD" = $1::text;`, [matricula]);
    }
